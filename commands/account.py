    ...
    async def account(self, ctx):
        ...
        if f'{user_id}' in user_data:
            ...
            def check(reactin, user):
                return user == ctx.message.author and str(reactin.emoji) == '🟩'

            try:
                reactin, user = await self.bot.wait_for(event='reaction_add', timeout=15.0, check=check)
        ...
        #     return user == ctx.message.author and str(reactin.emoji) == '🟥'
        # try:
        #     reactin, user = await self.bot.wait_for(event='reaction_add', timeout=60.0, check=check)
        # except asyncio.TimeoutError:
        #     await ctx.channel.send("어음.....")
        # else:
        #     embed = discord.Embed(color= 0x00ff9c)
        #     embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/773727937069056000/857254590218371082/526_B39FCE5.png")
        #     embed.add_field(name="파이", value="가입 하지 않으면 이용할떄 제약이 있서요ㅠ", inline=True)
        #     await msg.edit(embed=embed)
